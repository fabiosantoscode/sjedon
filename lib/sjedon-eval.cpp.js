...
ExecutionContext.prototype._evalExpression = function (expr) {
    ...
    if (expr.type === 'CallExpression' || expr.type === 'NewExpression') {
        this.pushOp('Call', expr);
        this.pushOp('Expression', expr.callee)

        var argumentsLeft = expr['arguments'].length;
        while (argumentsLeft--) {
            this.pushOp('Expression', expr['arguments'][argumentsLeft]);
        }
        ...
        this.pushOp('PointerGet')
        this.pushOp('PointerCreate', expr)
...
ExecutionContext.ops = {
    ...
    'Call': function (ast) {
        ...
        var argc = ast['arguments'].length;
        var argv = new Array(argc);
        while (argc--) {
            argv[argc] = this.stack.pop();
        }
        if (ast.type === 'CallExpression') {
            this.stack.push(
                this._evalCall(ast.callee, undefined, argv))
        } else if (ast.type === 'NewExpression') {
            var newObj = inheritFrom(callee.prototype || {});
            this._evalCall(callee, newObj, argv)
    ...
    'PointerGet': function (addr) {
        this.stack.push(this.stack.pop().getValue())
    },
    'PointerCreate': function (addr) {
        ...
        if (addr.type === 'Identifier') {
            ...
            this.pushOp(addr.computed ?
                'Expression' :
                'MaybeIdentifier', addr.property)
        } else {
...
ExecutionContext.prototype._evalCall = function (callee, context, args) {
    ...
    if (func.$Sjedon === this.sjedon) {
        return this.sjedon._evalFunctionCall(func.ast, func.closure, context || func.context, args)
    }
