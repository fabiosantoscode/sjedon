...
Sjedon.prototype.callFunction = function (callee, context, args) {
    ...
    if (callee.type === 'Identifier' || callee.type === 'MemberExpression') {
        ...
        if (callee.type === 'Identifier') {
            ...
            func = context[callee.property.type === 'Identifier' ?
                callee.property.name : this.evalExpression(callee.property)];
        }
        assert(func)
        if (typeof func === 'function') {
            func.apply(context, args);
        } else {
            this.runFunction(func, context, args)
        }
    } else if (callee.type === 'FunctionExpression' || callee.type === 'FunctionDeclaration') {
        return this.runFunction(callee, context, args);
    } else {
