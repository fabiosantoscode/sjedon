...
// Abstract class for things inside the machine
Sjedon.Object = function() {}


Sjedon.Function = function SjedonFunction(options) {
    this.ast = options.ast
    this.sjedon = options.sjedon
    this.length = options.ast.params.length
    Sjedon.Object.call(this);
}

Sjedon.Function.prototype = new Sjedon.Object();

Sjedon.Function.prototype.wrap = function () {
    var self = this;
    return function sjedonWrapper() {
        return self.sjedon.runFunction(self.ast, this, arguments);
    }
}
...
StackFrame.prototype.assignVar = function (name, value) {
    ...
    throw new Error('undefined reference: ' + name)  // TODO Actual ReferenceError
}
...
Sjedon.prototype.evalExpression = function (expr) {
    assert(expr, 'Sjedon#evalExpression: pass an expression!')
    if (expr.type === 'CallExpression') {
        return this.callFunction(expr.callee, null, expr['arguments'].map(this.evalExpression.bind(this)));
        ...
        return new Sjedon.Function({ ast: expr, sjedon: this });
    } else if (expr.type === 'AssignmentExpression') {
        var left = expr.left;
        if (left.type === 'Identifier') {
            return this.currentFrame.assignVar(left,
                this.evalExpression(expr.right))
        } else if (left.type === 'MemberExpression') {
            return this.evalExpression(left.object)[this.evalExpression(left.property)] = this.evalExpression(expr.right)
        } else {
...
Sjedon.prototype.callFunction = function (callee, context, args) {
    if (callee.type === 'Identifier') {
        var func = this.currentFrame.fetchVar(callee.name)
        assert(func)
        if (typeof func === 'function') {
            for (var i = 0; i < args.length; i++) {
                if (args[i] instanceof Sjedon.Object) { 
                    args[i] = args[i].wrap();
                }
            }
            func.apply(null, args);  // TODO context
        } else {
            this.runFunction(func, null, args)
        }
    } else if (callee.type === 'FunctionExpression' || callee.type === 'FunctionDeclaration') {
        return this.runFunction(callee, null, args);
    } else {
        notimplemented('calling functions other than FunctionExpression (given ' + callee.type + ')')
    }
}

Sjedon.prototype.runFunction = function (functionAST, context, args) {
