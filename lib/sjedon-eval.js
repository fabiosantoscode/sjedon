...
module.exports = function installEval(Sjedon, StackFrame) {

Sjedon.prototype._eval = function (block) {
    var ret = this._evalBlock(block || this.ast.body);
    if (ret !== Sjedon.nothing) { return ret; }
}


Sjedon.prototype._evalBlock = function (body) {
...
Sjedon.prototype._evalStatement = function (statement) {
    var ret;
    var self = this;
    ...
    if (statement.type === 'ExpressionStatement') {
        ...
        for (;i < len; i++) {
            ...
            if (ret === Sjedon.nothing) {
                continue;
            } else if (ret === Sjedon.break_) {
                return Sjedon.nothing;
            } else if (ret === Sjedon.continue_) {
                continue;
            } else {
                return ret;
            }
        ...
        statement.declarations.forEach(function (decl) {
            if (decl.init) {
                ...
                self.currentFrame.declareVar(decl.id.name, initial);
        ...
        this.currentFrame.assignVar(statement.id.name, this._evalExpression(statement))
        return Sjedon.nothing;
...
Sjedon.prototype._evalAssignment = function (expr) {
    var left = expr.left;
...
Sjedon.prototype._evalTernaryExpression = function (test, consequent, alternate) {
    return this._evalExpression(this._evalExpression(test) ? consequent : alternate)
}

Sjedon.prototype._evalPropertyAccess = function (expr) {
    if (!expr.computed) {
...
Sjedon.prototype._evalExpression = function (expr) {
    ...
    if (expr.type === 'SjedonQuotedExpression') {
        ...
        return this._evalFunction(expr, this.currentFrame);
        ...
        if (expr.operator !== 'delete') {
            return this.unaryExpression(expr.operator, this._evalExpression(expr.argument));
        } else {
...
Sjedon.prototype._evalCall = function (callee, context, args) {
    args = args || [];
    var func;
...
Sjedon.prototype._evalFunctionCall = function (functionAST, closure, context, args) {
    ...
    var ret = this._evalStatement(functionAST.body)

    this.currentFrame = stackFrame.parent

    return ret === Sjedon.nothing ?
        undefined :
        ret;
}

Sjedon.prototype._evalFunction = function (func, closure) {
    ...
    return function (_) {
        ...
        return self._evalFunctionCall(func, closure, this, arguments);
    }
}
